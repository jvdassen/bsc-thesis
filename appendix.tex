...
\section{Installing and Running a Bazo node on Android}
...
After this step the binaries can be run from the command line. This demonstrates how a miner application and a client can be started so that the web interface is running on port 8001:
\begin{framed}./bazo\_miner new\_database :8000 \&

./bazo\_client \&
\end{framed}
Now, the web service will be listening on port 8001 of the android device, so that this URL can be used in the Wallet application. Navigate to the settings page of the Wallet to modify the URI that will be used for all blockchain interaction.
