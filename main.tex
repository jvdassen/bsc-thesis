...
In order to communicate Bazo Transactions to other devices and applications, an URI Scheme was designed to hold transaction information, so that a requesting party can communicate it. The grammar and structure for the schema followed the \cite{bip21} 21st Bitcoin Improvement Proposal for an URI scheme since it contained all necessary elements to fulfill the requirement.
...
\underbrace{\overbrace{?amount=123}^{\mathrm{amoun t}}}_{\mathrm{optional components}}
\]
...
% reference implementation of decoder/encoder
